    ...
    Shader "ShaderMan/CandleFlameShader"
	{
	Properties{
...
float N( float x ) { return frac(sin(x*12.35462)*5346.1764); }
float N2(float x, float y) { return N(x + y*23414.324); }

float N21(float2 t) {return frac(sin((t.x+t.y*10.)*9e2));}
float N31(float3 t) {return frac(sin((t.x+t.y*10.+ t.z*100.)*9e2));}
float4 N14(float t) {return frac(sin(vec4(1., 3., 5., 7.)*9e2));}

float LN(float x) {return lerp(N(floor(x)), N(floor(x+1.)), frac(x));}
...
ray GetRay(float2 uv, float3 p, float3 lookAt, float zoom) {
    ...
    cr.o = p;						
    cr.d = normalize(i-p);		// ray dir is floattor from cam pos to screen intersect 
	return cr;
}
...
float2 smin( float a, float b, float k )
{
    ...
    return vec2(lerp( b, a, h ) - k*h*(1.0-h), h);
}
float2 smin2( float a, float b, float k, float z )
{
    float h = clamp( 0.5+0.5*(b-a)/k, 0.0, 1.0 );
    float g = h*h*(3.-2.*h);
    return vec2(lerp( b, a, g ) - k*pow(g*(1.-g), z), g);
}

float2 smax( float a, float b, float k )
{
	float h = clamp( 0.5 + 0.5*(b-a)/k, 0.0, 1.0 );
	return vec2(lerp( a, b, h ) + k*h*(1.0-h), h);
}
...
float sdCylinder( float3 p, float2 h ) {
  float2 d = abs(vec2(length(p.xz),p.y)) - h;
  return min(max(d.x,d.y),0.0) + length(max(d,0.0));
}

float sdCone( float3 p, float3 c ) {
    float2 q = vec2( length(p.xz), p.y );
    float d1 = -q.y-c.z;
...
float3 Bend( float3 p, float center, float strength ) {
    ...
    float2 tmp = m*vec2(p.x, p.y);
    float3  q = vec3(tmp.x,tmp.y,p.z);
    q.y+=center;
    return q;
}
...
for(float i=0.; i<BG_STEPS; i++) {
        ...
        float3 cP = vec3(0, N21(id)*CANDLE_HEIGHT, 0);
...
float3 render( float2 uv, ray cam ) {
    ...
    if(o.m==-1.)
        ...
        float3 sssColor = lerp(lerp(vec3(1), vec3(1, 0, 0), o.b), CANDLECOL, adult);
    ...
    float t = frac(_Time.y/SONG_LENGTH)*SONG_LENGTH;
    
    uv = (2.*uv - (o.xy=1) ) / o.y ;  	// -1 <> 1
   	m = _iMouse.xy/1;					// 0 <> 1
